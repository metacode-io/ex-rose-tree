defmodule ExRoseTree.ZipperTest do
  use ExUnit.Case, async: true
  use ZipperCase

  doctest ExRoseTree.Zipper

  @bad_zippers [
    {%{focus: %ExRoseTree{term: 1, children: []}, prev: [], next: [], path: []}, 0},
    {true, 1},
    {false, 2},
    {5, 3},
    {"a word", 4},
    ...
    {%Zipper{focus: %ExRoseTree{term: 1, children: []}, prev: :not_a_list, next: [], path: []},
     10},
    {%Zipper{focus: %ExRoseTree{term: 1, children: []}, prev: [], next: :not_a_list, path: []},
     11},
    {%Zipper{focus: %ExRoseTree{term: 1, children: []}, prev: [], next: [], path: :not_a_list},
     12},
    {nil, 13}
  ]
  ...
  describe "has_siblings?/1 guard" do
    test "should return true when given a Zipper who has only previous ExRoseTree siblings", %{
      z_with_siblings: z
    } do
      new_z = %{z | next: []}
    ...
    test "should return true when given a Zipper who has only next ExRoseTree siblings", %{
      z_with_siblings: z
    } do
      new_z = %{z | prev: []}
      assert Zipper.has_siblings?(new_z) == true
    ...
    test "should return true when given a Zipper who has both previous and next ExRoseTree siblings",
         %{z_with_siblings: z} do
      assert Zipper.has_siblings?(z) == true
    end

    test "should return false when given a Zipper who has neither previous or next ExRoseTree siblings",
         %{simple_z: z} do
